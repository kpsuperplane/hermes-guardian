...
def test_tool_override_slash_requires_owner():
    ...
    plugin._remember_command_owner("tool set evil egress=ignore", stranger)
    message = plugin._handle_guardian_command("tool set evil egress=ignore")
    assert "Permission denied" in message


def test_tool_override_slash_roundtrip_for_cli_owner():
    plugin = load_plugin()
    plugin._remember_command_owner("tool set mcp_acme_* egress=ignore", plugin._CLI_OWNER_HASH)
    message = plugin._handle_guardian_command("tool set mcp_acme_* egress=ignore")
    assert "Saved tool override" in message
    plugin._remember_command_owner("tools", plugin._CLI_OWNER_HASH)
    listing = plugin._handle_guardian_command("tools")
