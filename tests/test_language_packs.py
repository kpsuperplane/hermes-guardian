...
def test_bundled_language_packs_validate():
    for pack_id in _ALL_PACK_IDS:
        pack = importlib.import_module(f"language_packs.{pack_id}").PACK
...
def test_language_pack_compiler_matches_scripts_without_spaces():
    pattern = _literal_phrase_pattern(["重置密码", "パスワードリセット"])

    assert pattern.search("请重置密码")
    assert pattern.search("パスワードリセットしてください")
...
def test_compile_language_packs_exposes_multilingual_patterns():
    compiled = _compile_language_packs("en,es,zh,ar,hi")
    ...
    assert compiled.auth_code_label_pattern.search("验证码")
    assert compiled.private_field_pattern.search("البريد الإلكتروني")
    assert compiled.security_link_term_pattern.search("सत्यापित")
