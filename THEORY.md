...
Because conservative classification produces false positives for genuinely benign custom tools, Guardian pairs it with operator-managed tool classifications. Reading classifications declare what a specific tool or tool-name prefix reads: which private classes its results carry, and whether document reads are reference material or personal data. Sharing classifications declare egress behavior: whether a tool is a safe non-sink, a forced-gate sink, or a specific action family and destination. These classifications are trusted for classification but are not declassification of access-sensitive content: they never bypass the non-approvable Security Module or the intrinsic same-call hard blocks.
