...
Guardian treats prompt injection as an expected failure mode and enforces at egress instead of trying to recognize malicious instructions (see the Abstract). This is a forced move, not a stylistic preference: three independent arguments hold that injection cannot be solved at the model layer. Conceptually, data/instruction separation is incoherent for an agent — an agent's context is instructional everywhere by design (memory, skills, tool results, third-party content are all read as guidance), so a defense cannot separate "data" from "instructions" without breaking the workflows it protects (Abdelnabi & Bagdasarian, 2026). Formally, an LLM-based detector is as injectable as the model it guards: known-answer detection is structurally unsound, with the DataFlip adaptive attack driving detection to 0% while keeping a 91% attack success rate, because the detector executes the injected task as its own signal (Choudhary et al., 2025). Empirically, a systematization across 78 studies finds adaptive attacks exceed 85% success against deployed defenses and names the root cause as the architectural conflation of code and data (Maloyan & Namiot, 2026). Unlike SQL injection, which was solvable by *separation*, LLM injection has no separate channel to separate into.
