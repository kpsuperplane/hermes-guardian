...
Hermes Guardian is a privacy-aware egress and declassification layer for Hermes Agent. Its distinctive contribution is the *asset* it protects, not only the mechanism it uses. Credentials, API keys, and most PII have a recognizable signature — they match patterns, which is what scanner, DLP, and secret-detection tools key on. The content a personal agent most needs to read — an email body, a contact list, a calendar, a Notion page — has no signature; it is private only because of where it came from. Guardian treats this content — private by its source rather than by what it looks like — as its primary protected asset, tracking it by origin so that data with no detectable shape is still governed at egress. (The compact name for this is *provenance-private*: private by source, not by signature. The term appears where it is convenient, but the idea is just "private because of where it came from.")
...
This document is the formal counterpart to the narrative argument in [When secrets don't look like secrets](https://kevinpei.com/posts/thoughts-on-agent-privacy), which makes the same case in plain, first-person terms; the rigor, threat model, and field positioning are developed here, and the reader who wants the origin story and the intuition should start there. The blog's title is precisely this thesis: the secrets that matter most are the ones that do not look like secrets, and a defense that can only see signatures cannot see them at all.
...
Guardian treats prompt injection as an expected failure mode and enforces at egress instead of trying to recognize malicious instructions (see the Abstract). This is a forced move, not a stylistic preference: three independent arguments hold that injection cannot be solved at the model layer. Conceptually, [Abdelnabi and Bagdasarian](https://arxiv.org/abs/2605.17634) argue that data/instruction separation is incoherent for an agent — an agent's context is instructional everywhere by design (memory, skills, tool results, third-party content are all read as guidance), so a defense cannot separate "data" from "instructions" without breaking the workflows it protects. Formally, [Choudhary et al.](https://arxiv.org/abs/2507.05630) show that an LLM-based detector is as injectable as the model it guards: known-answer detection is structurally unsound, with the DataFlip adaptive attack driving detection to 0% while keeping a 91% attack success rate, because the detector executes the injected task as its own signal. Empirically, [Maloyan and Namiot's SoK](https://arxiv.org/abs/2601.17548) synthesizes 78 studies and finds adaptive attacks exceed 85% success against deployed defenses, with the root cause named as the architectural conflation of code and data — the same property that made SQL injection solvable by *separation* makes LLM injection unsolvable, because the model has no separate channel to separate into.
...
What *taints* on a document read is tiered by where the read came from, not left as an undifferentiated content scan. The scan for document reads is deliberately lenient, because skill docs are reference material full of placeholder contacts (`you@example.com`, a `555` sample number) that would otherwise gate unrelated egress for the rest of the turn. That leniency is safe only for operator-installed reference material; extended to *any* read by tool-name shape it would be a silent fail-open, leaving an arbitrary MCP server's genuinely personal content untainted for lack of a structural signal. Guardian therefore tiers the source: a provably-reference read (the `skill_view` builtin, or a path resolving under the skills tree) keeps the lenient scan; a read from a server the operator has *declared* (`source = reference`, `source = private`, or `source = unknown`) follows the declaration; an *undeclared* MCP document read of unknown origin fails closed, tainting `documents` regardless of detectable signal, with a one-time per-server classification suggestion so the operator can declare it once. `source = unknown` is a remembered unresolved classification: it suppresses repeated review but does not relax the fallback taint.

For arbitrary unknown non-MCP reads, Taint Classification chooses the fallback. `balanced` preserves best-effort source inference plus content scanning, so mundane private prose from an unknown tool can remain untainted unless the operator declares the tool or the content carries a detectable private structure. `strict` treats otherwise-unknown non-MCP read results as `documents` when no stronger classification applies. `relaxed` keeps balanced read inference but does not gate unrecognized non-MCP tools under taint, and is surfaced as a runtime risk. This gives operators a choice between lower-noise read inference, broader unknown-source conservatism, and an explicit permissive mode for custom tool-heavy environments.

Guardian can also use an enabled-by-default LLM source classifier for otherwise-unknown, signalless reads. Unlike the egress verifier, this classifier never receives payload contents: it sees only metadata such as tool name, matcher shape, argument keys/types, status, result type/size, and JSON keys. Its output is not a transient verdict; it writes a normal Reading tool classification as `reference`, `private`, or `unknown`, so the same tool matcher does not need to be reviewed again. The `unknown` output is deliberately useful: the model can remember that it could not establish where the data came from without pretending the source is safe.

This coarseness is conservative in one direction and imprecise in another:

- It can block harmless outgoing content, because the full ambient session taint applies to any outward flow absent a verifier allow or a declassification rule.
...
Guardian therefore classifies conservatively in both directions. In balanced and strict Taint Classification, a tool that matches no known built-in family is treated as an unrecognized sink and gated under taint (the `tool_unknown` family), mirroring the treatment of unknown MCP tools, rather than being allowed by default. The operator can opt into permissive handling with relaxed Taint Classification, but that choice is explicit and surfaced as a runtime risk.

A browser console eval is classified by what its expression does, not by the tool name alone. Reading page state — DOM nodes, form field values, page text, attributes — and returning it to the agent is a read, not a sink: the agent already has page read access through the ungated read tools, and any later attempt to send that data onward is independently gated. Guardian recognizes this through a fail-closed allowlist — the eval is a read only when every operation is a known pure-read accessor — so anything it cannot prove read-only falls through to gating. An eval that writes into the page (assigning to DOM/element properties, inserting nodes, setting attributes), submits, navigates, reads a credential store, runs dynamic code, or hits a network sink is a sink. The page-write case matters even with no network call: writing tainted data into an attacker-controlled DOM is an exfiltration channel, because page-resident script can read the mutation back out. This is the same "reading is not egress" principle applied to in-page operations.

A turn-scoped deterministic rule — the cross-channel lockdown — complements the per-action decision: once a private outward export has been withheld in a turn, Guardian records a volatile, metadata-only reroute guard. A later action still receives its normal read-only or verifier judgment, but an otherwise auto-approved retry is downgraded to manual review when it matches the withheld flow by overlapping policy classes plus route metadata such as destination, action family, purpose, recipient identity, or exact argument fingerprint. High-risk denials and intrinsic exfiltration shapes arm a broader same-turn guard for overlapping private classes, because those are the clearest channel-shopping attempts; ordinary ambiguity is route-scoped so one denied item in a bulk owner-authorized workflow does not freeze unrelated destinations. A task-driven agent's natural response to a block is to re-route (a gated terminal export re-tried through a browser form), and the lockdown denies that re-route a softer channel, so the human review the first block requested cannot be shopped around. The lockdown state is volatile and clears on the next owner message; explicit approvals and standing rules still apply, since they are themselves the human review the lockdown preserves.

Because conservative classification produces false positives for genuinely benign custom tools, Guardian pairs it with operator-managed tool classifications. Reading classifications declare what a specific tool or tool-name prefix reads: which private classes its results carry, and whether document reads are reference material, personal data, or still unknown. Sharing classifications declare egress behavior: whether a tool is a safe non-sink, a forced-gate sink, or a specific action family and destination. These classifications are trusted for classification but are not declassification of access-sensitive content: they never bypass the non-approvable Security Module or the intrinsic same-call hard blocks.
...
The Security Module is configured through high-level rules that are enabled by default. These rules cover semantic account-security content, credential-shaped content, sensitive links, intrinsic same-call exfiltration shapes, and terminal remote-read shortcuts targeting private-network or metadata hosts. Intrinsic same-call detection is structural: it looks for local/code secret reads, browser console/CDP state reads, or obvious MCP private-source tools combined with network, webhook, or share sinks. When an enabled Security Module rule produces a finding, the action, tool result, or final response is blocked or suppressed without an approval path. Privacy allow rules, Egress Safety changes, and approval commands do not declassify Security Module findings.

Credential suppression is directional, consistent with the egress-first thesis. Hard-secret categories — private keys, cloud access keys, and password or private-key assignments — are suppressed on every surface, inbound tool results included. Account-security content is also suppressed inbound when it carries concrete access-sensitive material such as OTP/MFA codes, reset/recovery links, magic links, redaction markers, or security notifications from ordinary sources. Provably-reference material (`skill_view` and reads under the operator-installed skills tree) gets a narrower inbound relaxation for phrase-only account-security prose: reference docs can discuss password reset, verification, magic-link, or security-alert concepts without being access material. API/service authentication tokens are treated asymmetrically: blocked on egress (tool arguments, gateway dispatch, final response) but read into context on the inbound tool-result path. Suppressing a service token at read-time would break legitimate integrations — an MCP server surfacing its own auth token — without preventing exfiltration: a token that never leaves has not leaked, and one the model later tries to send is still caught at the sink. Reading is not egress, so the confidentiality boundary stays where Guardian enforces it.

Security-rule toggles are administrative model changes, not declassification decisions. Disabling a Security Module rule means Guardian no longer categorically blocks that content or action shape. The privacy taint-and-egress layer can still gate classified private egress, but it is no substitute for the disabled non-approvable hardening category. Runtime policy snapshots, `/guardian status`, and the dashboard surface risk banners for concrete risky Guardian configuration such as disabled intrinsic same-call hardening.
...
Under these assumptions, Guardian is no longer the sole safety layer; it is the semantic egress layer in a broader containment architecture.
...
Guardian is squarely in the second family, for the reason set out under *Background*: if injection cannot be solved at the model layer, a defense whose decision routes through model judgment inherits the same foolability it is trying to defend against. So the first family, whatever its utility for a cooperative agent, cannot be the security boundary for an adversarial one; that boundary has to live in family two.
...
Stated in the field's own coordinates: Guardian is in the deterministic-enforcement family; it implements the governance parameters (traces, policies, compliance) that the descriptive-and-measurement majority leaves empty; it sides with the injection-pessimists and the CI-skeptics in the field's central disagreement; and it anchors on owner authorization rather than receiver-context norms. The parameters it does not implement — policy combination, receiver-context norms, the full normative heuristic — are not places it lags the field; they are hard for everyone or out of scope for a single-owner mechanism. The contribution, if it has one, is less "a better CI system" than a worked example of what remains buildable and sound when you stop relying on model judgment entirely: owner-authorized, trace-aware boundary enforcement, scoped honestly to what it guarantees.
...
Its primary protected asset is personal content that is private by where it came from — confidential because of its source rather than because it matches a sensitive pattern — which is the half of the problem that signature-based scanners and DLP cannot see.
...
- Abdelnabi and Bagdasarian, “AI Agents May Always Fall for Prompt Injections” (data/instruction separation is incoherent for agents; recasts injection via contextual integrity): <https://arxiv.org/abs/2605.17634>
- Choudhary et al., “How Not to Detect Prompt Injections with an LLM” (formal unsoundness of known-answer detection; the DataFlip attack): <https://arxiv.org/abs/2507.05630>
- Maloyan and Namiot, “Prompt Injection Attacks on Agentic Coding Assistants” (SoK; >85% adaptive attack success across 78 studies; code/data conflation): <https://arxiv.org/abs/2601.17548>
